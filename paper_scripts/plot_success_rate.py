...
def remap_to_file_order(worlds_arr, rates_or_means, world_order,
                        stds=None, show_world_ids=False):
    """
...
def plot_success_rates(files, labels=None, max_world=None, output=None,
                       task_num=None, world_order=None, world_set=None,
    ...
    fig.suptitle("Success Rate per World",
                 fontsize=int(15 * font_scale), fontweight="bold")
    ...
    for idx, (fpath, label) in enumerate(zip(files, labels)):
        ...
        ax_s.bar(x_pos, rates, width=0.8, color=color, alpha=0.85,
        ...
        ax_s.set_title(f"{label} — Success Rate",
                       fontsize=int(11 * font_scale), fontweight="semibold", loc="left")
        ...
        ax_c = axes[idx][1]
        worlds_c, means, stds = compute_clearance_stats(world_clearance, max_world, world_set)

        if world_order is not None:
            x_pos_c, means, stds, tick_labels_c = remap_to_file_order(
                worlds_c, means, world_order, stds=stds,
                show_world_ids=show_world_ids
            )
        else:
            x_pos_c, tick_labels_c = worlds_c, None

        ax_c.bar(x_pos_c, means, width=0.8, color=color, alpha=0.75,
                 edgecolor="white", linewidth=0.4)
        ax_c.errorbar(x_pos_c, means, yerr=stds, fmt="none",
        ...
        ax_c.set_title(f"{label} — Min Clearance to Obstacle",
                       fontsize=int(11 * font_scale), fontweight="semibold", loc="left")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--show_world_ids", action="store_true",
    ...
    plot_success_rates(args.files, args.labels, args.max_world, args.output,
                       args.task_num, world_order, world_set,
