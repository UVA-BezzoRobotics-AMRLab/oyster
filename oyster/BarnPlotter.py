...
class BarnPlotter:
    ...
    def init_plot(self, curve, occ_grid, upper_coeffs, lower_coeffs, mpc):
        ...
        self.fig.subplots_adjust(left=0.04, right=0.97, top=0.93,
                                 bottom=0.08, wspace=0.32, hspace=0.55)
        ...
        _style_ax(self.ax_alphas, title="α  values",
                  xlabel="step", ylabel="α", zero_line=False)
        _style_ax(self.ax_cbfs,   title="CBF  h(x)",
                  xlabel="step", ylabel="h(x)", zero_line=True)
        _style_ax(self.ax_reward, title="velocity",
        ...
        (self.alpha_upper_line,) = self.ax_alphas.plot(
            ...
            path_effects=[pe.Stroke(linewidth=lw+1.5, foreground=DARK_BG), pe.Normal()])
        (self.alpha_lower_line,) = self.ax_alphas.plot(
            [], [], color=C_LOWER,  linewidth=lw, label="α lower",
            path_effects=[pe.Stroke(linewidth=lw+1.5, foreground=DARK_BG), pe.Normal()])

        self.ax_alphas.legend(loc="upper right", framealpha=0.6)

        (self.cbf_upper_line,) = self.ax_cbfs.plot(
            [], [], color=C_UPPER, linewidth=lw, label="h above",
            path_effects=[pe.Stroke(linewidth=lw+1.5, foreground=DARK_BG), pe.Normal()])
        (self.cbf_lower_line,) = self.ax_cbfs.plot(
            [], [], color=C_LOWER,  linewidth=lw, label="h below",
            path_effects=[pe.Stroke(linewidth=lw+1.5, foreground=DARK_BG), pe.Normal()])

        # zero-crossing shading (filled between h and 0)
        self.cbf_upper_fill = self.ax_cbfs.fill_between([], [], 0,
            color=C_UPPER, alpha=0.12, zorder=0)
        self.cbf_lower_fill = self.ax_cbfs.fill_between([], [], 0,
            color=C_LOWER,  alpha=0.12, zorder=0)

        self.ax_cbfs.legend(loc="upper right", framealpha=0.6)

        (self.reward_line,) = self.ax_reward.plot(
            [], [], color=GREEN, linewidth=lw, label="velocity",
            path_effects=[pe.Stroke(linewidth=lw+1.5, foreground=DARK_BG), pe.Normal()])

        # gradient fill under reward
        self.reward_fill = self.ax_reward.fill_between([], [], 0,
        ...
        # reference point
        (self.ref_point,) = self.ax.plot([], [], "o", color=C_HORIZON,
                                          markersize=5, label="reference", zorder=6)

        # robot path trail
        (self.path_line,) = self.ax.plot([], [], color=C_PATH,
                                          linewidth=1.5, alpha=0.7,
                                          linestyle="-", zorder=4,
                                          label="trail")

        # MPC horizon
        (self.mpc_horizon,) = self.ax.plot([], [], color=C_HORIZON,
                                            linewidth=4.2, alpha=0.8,
                                            zorder=5, label="MPC horizon")

        # reference trajectory from MPC belief
        (self.mpc_trajectory_belief,) = self.ax.plot([], [], color=WHITE,
                                                       linewidth=1.2,
                                                       alpha=0.4,
                                                       linestyle="--",
                                                       zorder=3)
    ...
    def _init_robot_patch(self):
        if self.dynamics == Dynamics.DOUBLE_INTEGRATOR:
            self.robot_patch = Circle(
                (0, 0), self.robot_size,
                facecolor=C_ROBO, edgecolor=WHITE,
    ...
    def init_tubes(self, curve, upper_coeffs, lower_coeffs, ax=None):
        ...
        (self.upper_tube_line,)   = ax.plot([], [], color=C_UPPER, **tube_kw,   label="tube upper")
        (self.lower_tube_line,)   = ax.plot([], [], color=C_LOWER,  **tube_kw,   label="tube lower")
        (self.upper_tube_belief,) = ax.plot([], [], color=C_UPPER, **belief_kw)
        (self.lower_tube_belief,) = ax.plot([], [], color=C_LOWER,  **belief_kw)
        (self.upper_tube_pts,)    = ax.plot([], [], "o", color=C_UPPER, markersize=5, zorder=4)
        (self.lower_tube_pts,)    = ax.plot([], [], "o", color=C_LOWER,  markersize=5, zorder=4)
    ...
    def plot_tubes(self, curve, robot_state, mpc, upper_coeffs, lower_coeffs):
        ...
        args  = {
            ...
            "i2": xs, "i3": ys,
            "i4": upper_coeffs, "i5": lower_coeffs,
            "i6": mpc.get_params()["CLF_W_LAG"],
            "i7": mpc.get_params()["CLF_W_CONTOUR"],
            "i8": mpc.get_params()["CLF_GAMMA"],
            "i9": traj_view.arclen,
        }
    ...
    def render(self, robot_state, current_ref, curve, mpc, upper_coeffs, lower_coeffs):
        ...
        self.alpha_upper_hist.append(params["CBF_ALPHA_ABV"])
        self.alpha_lower_hist.append(params["CBF_ALPHA_BLW"])

        self.alpha_upper_line.set_data(self.steps, self.alpha_upper_hist[:len(self.steps)])
        self.alpha_lower_line.set_data(self.steps, self.alpha_lower_hist[:len(self.steps)])
    ...
    def get_cbfs(self, mpc, upper_coeffs, lower_coeffs):
        ...
        return float(mpc.debug_fns["h_abv"](**args)), \
               float(mpc.debug_fns["h_blw"](**args))
